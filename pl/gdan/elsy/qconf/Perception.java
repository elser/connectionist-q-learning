...
public abstract class Perception implements Serializable {
    ...
    protected CuriousPlayerPerception inputPerception;
    ...
    protected void setNextValue(double input) {
        if (output != null) {
            if (isUnipolar()) {
                output[outputIter] = MatOld.sigmoidUni(input);
            } else {
                output[outputIter] = MatOld.sigmoidBi(input);
            }
        }
        outputIter++;
    }
    ...
    public void setInputPerception(CuriousPlayerPerception perception) {
        this.inputPerception = perception;
    }
